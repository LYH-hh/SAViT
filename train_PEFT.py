...
import Model_SAViT_PEFT

from engine_train import train_one_epoch, evaluate


def get_args_parser():
    parser = argparse.ArgumentParser('SAViT for medical image classification', add_help=False)
    ...
    # Model parameters
    parser.add_argument('--model', default='SAViT_base_patch16', type=str, metavar='MODEL',
    ...
    # * Finetuning params
    parser.add_argument('--finetune', default='/lyh/code/checkpoints/vit_base_patch16_224_in21k.pth',type=str,
    ...
    parser.add_argument('--task', default='./output_dir/SAViT_PEFT/base/ChildCXR_noCrossAttn_scale/',type=str,
                        help='finetune from checkpoint')
    parser.add_argument('--output_dir', default='./output_dir/SAViT_PEFT/base/ChildCXR_noCrossAttn_scale',
                        help='path where to save, empty for no saving')
    parser.add_argument('--log_dir', default='./output_dir/SAViT_PEFT/base/ChildCXR_noCrossAttn_scale/',
...
def main(args):
    ...
    model = Model_SAViT_PEFT.__dict__[args.model](
    ...
    if args.finetune and not args.eval:
        ...
        for name in list(checkpoint_model.keys()):
            change_name = name.replace('blocks.', "spa.blocks.")
            #change_name = name.replace('spa.blocks.11.', "mixpool.theta_mp.")
            checkpoint_model[change_name] = checkpoint_model.pop(name)
        for name in list(checkpoint_model.keys()):
            #change_name = name.replace('blocks.', "spa.blocks.")
            change_name = name.replace('spa.blocks.11.', "mixpool.theta_mp.")
            checkpoint_model[change_name] = checkpoint_model.pop(name)
